	...
	if (confId) {
		...
		if (selectedConf != null && selectedConf.id == confId) {
			...
			$.ajax({
				...
				url : '/xkeng/conference/' + confId + '/slots',
...
ProxyAPI.loadConfSummary = function(pastcount, futurecount) {
	$.ajax({
		...
		dataType : 'json',
		contentType : 'application/json',
		url : '/xkeng/conferences/summary/pastcount/' + pastcount
...
ProxyAPI.commentSession = function(sessId, comment, callback) {
	$.mobile.showPageLoadingMsg();
	var jsonRequest = {};
	jsonRequest.comment = comment;
	$.ajax({
		type : 'POST',
		dataType : 'json',
		contentType : 'application/json',
		url : '/xkeng/feedback/' + sessId + '/comment',
		success : callback,
		crossDomain : false,
		data : JSON.stringify(jsonRequest),
		error : handleException
